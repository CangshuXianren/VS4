...
bool ifAchieved(custom_messages::vehicle_status now_pos, custom_messages::vehicle_status ref_pos)
{
    ...
    if(dis <= 1.5*overall_length) return true;
    return false;
}
...
void UpdateRefPoint(custom_messages::vehicle_status &instate, custom_messages::vehicle_status &incmd, const std::vector<quadprogpp::pathpoint> &refTrajectory){
    instate.xPos = refTraj[idx].x_r; // 更新参考轨迹中的跟踪点
...
int main(int argc, char  **argv)
{
    ...
    while (ros::ok())
    {
        ...
        // std::cout << "refTraj size : "<< refTraj.size()<<std::endl;
        UpdateRefPoint(now_pos, Aim_con, refTraj);
        while (ifAchieved(now_pos))
        {
            UpdateRefPoint(now_pos, Aim_con, refTraj);
        ...
        if(idx < refTraj.size()){
            ref_pos.xPos = refTraj[idx].x_r; // 更新参考轨迹中的跟踪点
            ref_pos.yPos = refTraj[idx].y_r;
            ref_pos.yaw = refTraj[idx].fai_r;
            Aim_con.speed = refTraj[idx].vx_r;
            Aim_con.angular_speed = refTraj[idx].vw_r;
            ...
            if(ifAchieved(now_pos, ref_pos)){
                ...
                std::cout << "[control] : distance : "<<sqrt(pow(now_pos.xPos - ref_pos.xPos, 2) + pow(now_pos.yPos - ref_pos.yPos, 2)) << ", tol : "<< 1.5*overall_length << std::endl;
        ...
        std::cout << "reference= x : "<< ref_pos.xPos << ", y : " << ref_pos.yPos << ", yaw : " << ref_pos.yaw << std::endl;
        std::cout << "current= x : " << now_pos.xPos << ", y : " << now_pos.yPos << ", yaw : " << now_pos.yaw << std::endl;
        std::cout << "refcmd = speed : " << Aim_con.speed << ", aspeed : " << Aim_con.angular_speed << std::endl;
        ...
        // geometry_msgs::Twist command;
        // command.linear.x=Bim_con.speed;
        // command.angular.z=Bim_con.angular_speed;   // xtark小车接收的cmd_vel中的角速度值直接当成前轮转角了，因此不能用这句
        // // command.angular.z=atan2(0.18*Bim_con.angular_speed,Bim_con.speed);    // 0.18是xtark的轴距，用atan2求转角
        // cmd_pub.publish(command);
    }
    return 0;
}