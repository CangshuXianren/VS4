...
void refTrajCallback(const custom_messages::planning_info::ConstPtr& trajectory_msg){
    if(trajectory_msg == nullptr || trajectory_msg->states.empty()){
        ROS_WARN("[control] : trajectory empty");
        return;
    }
    refTraj.clear();
...
bool ifAchieved(custom_messages::vehicle_status now_pos, custom_messages::vehicle_status ref_pos)
{
    ...
    if(dis <= gap_coef*overall_length) return true;
    return false;
}
...
int main(int argc, char  **argv)
{
    ...
    nh.param("mpc_node/mpc_hz", mpc_hz, 50.0);
    nh.param("gap_coef", gap_coef, 0.1);
