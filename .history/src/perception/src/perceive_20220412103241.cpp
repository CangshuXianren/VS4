...
void visObstacle(vector<Eigen::Vector3d> nodes){
    ...
    {
        std::cout<<"dsfasdfdsafffffffffffffffffffffffffffffff"<<std::endl;
        ...
        node_vis.scale.x = 2*nodes[i](2); // marker的尺寸，在位姿/方向之前应用，[1,1,1]意思是尺寸是1m*1m*1m
...
// 			 coefficients->values[0], coefficients->values[1], coefficients->values[2], coefficients->values[3]);
    ...
    for(auto &my : obj){
        ...
        tmpvis(0) = now_pos.xPos + my.x;
        tmpvis(1) = now_pos.yPos + my.y;
...
void TestCallback(const custom_messages::mapmodel::ConstPtr& perceive_msg){
    custom_messages::mapmodel tmpmapmodel;
    vector<Eigen::Vector3d> obs_vis;
    Eigen::Vector3d tmpvis;
    for(auto &my : tmpmapmodel.mapinfo){
        tmpvis(0) = now_pos.xPos + my.x;
        tmpvis(1) = now_pos.yPos + my.y;
        tmpvis(2) = my.z;
        obs_vis.push_back(tmpvis);
...
int main(int argc, char** argv)
{
    ...
    obs_vis_pub = nh.advertise<visualization_msgs::Marker>("dynamic_obs_vis", 1);
    ros::spin();
