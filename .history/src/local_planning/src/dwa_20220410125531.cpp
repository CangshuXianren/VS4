...
void DWA::FLPathCallback(const custom_messages::vehicle_status::ConstPtr& path_msg){
    ...
    tmp.pose.position.x = path_msg->xPos;
    tmp.pose.position.y = path_msg->yPos;
...
void DWA::planning()
{
    ...
    std::cout<<"dsafadsfadsfads"<<std::endl;
    vector<double> speed(2);     //v[0]为速度, v[1]角速度
    speed = dwa_control(currentState);
    cout << "final speed:(" << speed[0] << ", " << speed[1] << ")" << endl;
    currentTrajectory.clear();
...
double DWA::calc_obstacle_cost(const vector<CarState> &trajectory, const CarState &carstate, const AstarPathFinder* tmpmap)
{
    ...
    pCar.x = carstate.x;
    pCar.y = carstate.y;
    int num = tree->radiusSearch(pCar, window * 0.5, k_indices, k_sqr_distances);
    if (num) {
        for (size_t i (0); i < k_indices.size(); i++) {
            pcl::PointXY p = mapCloud->points[k_indices[i]];        
        }
        for (int k = 0; k < trajectory.size(); k++) {
            distance = sqrt(pow(carstate.x - trajectory[k].x, 2) + pow(carstate.y - trajectory[k].y, 2));
