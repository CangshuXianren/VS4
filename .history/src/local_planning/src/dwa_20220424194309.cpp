...
vector<double> DWA::calc_best_speed(const CarState &carstate, const vector<double> &dw)
{
    ...
    for(double i = dw[0]; i < dw[1]; i += v_resolution)
    {
        for (double j = dw[2]; j < dw[3]; j += yaw_rate_resolution)
        {
            ...
            if(CollisionCheck(trajectoryTmp, carstate, dwamap, avoid_mode)){
                continue;
            }
...
// 计算障碍代价
bool DWA::CollisionCheck(const vector<CarState> &trajectory, const CarState &carstate, const AstarPathFinder* tmpmap, int mode)
{
    double distance;
    if(mode){
