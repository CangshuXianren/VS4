    ...
    if(!first){
        ...
        while(CheckDWAArrive(currentState, myend)){
            if(++iteration < ends.poses.size()){
                ...
                custom_messages::planning_info empty_trajectory;
                geometry_msgs::PoseArray empty_trajectory_vis;
                trajectory_vis_pub.publish(empty_trajectory_vis);
                trajectory_pub.publish(empty_trajectory);
