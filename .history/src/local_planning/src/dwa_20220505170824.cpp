...
void DWA::FLPathCallback(const nav_msgs::Odometry::ConstPtr& path_msg){
    ends.poses.clear();
    geometry_msgs::PoseStamped tmp;
    tmp.pose.position.x = path_msg->pose.pose.position.x;
    tmp.pose.position.y = path_msg->pose.pose.position.y;
    tmp.pose.orientation.w = path_msg->pose.pose.orientation.w;
    tmp.pose.orientation.x = path_msg->pose.pose.orientation.x;
    tmp.pose.orientation.y = path_msg->pose.pose.orientation.y;
    tmp.pose.orientation.z = path_msg->pose.pose.orientation.z;
...
{
    if(ends.poses.empty()){
        std::cout << "[local planning] : No target" << std::endl;
        return;
    }
    if(!FLinit){
        std::cout << "[local planning] : No initial" << std::endl;
        return;
    }
    ...
    tf::Quaternion quat;
    tf::quaternionMsgToTF(ends.poses.front().pose.orientation, quat);
    double tmproll, tmppitch, tmpyaw;
    tf::Matrix3x3(quat).getRPY(tmproll, tmppitch, tmpyaw);
    switch (formation_type)
    {
    case 0:
        myend.x = ends.poses.front().pose.position.x - gap * cos(tmpyaw);
        myend.y = ends.poses.front().pose.position.y - gap * sin(tmpyaw);
        break;
    ...
    // if(CheckDWAArrive(currentState, currentTrajectory.back(), 0.5)){
    //     return;
    // }
...
int main(int argc, char** argv){
    ...
    char strID = '0' + ID - 1;
    string FLtarget = (string)"/No_" + strID + string("/myodom");
    string FLtarget_cmd = (string)"/No_" + strID + string("/cmd_vel");

    ros::Subscriber front_sub = n.subscribe("front_cmd_topic", 10, FrontCmdCallback);
    ...
    if(!ID){
        ...
        FLpath_sub = n.subscribe(FLtarget, 1, &DWA::FLPathCallback, mydwa);
    }
