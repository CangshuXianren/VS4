...
vector<double> DWA::calc_best_speed(const CarState &carstate, const vector<double> &dw)
{
    ...
    for(double i = dw[0]; i < dw[1]; i += v_resolution)
    {
        for (double j = dw[2]; j < dw[3]; j += yaw_rate_resolution)
        {
            ...
            speed_cost = speed_cost_gain * (dw[1] - trajectoryTmp.back().speed) / dw[1];
            obstacle_cost = obstacle_cost_gain * calc_obstacle_cost(trajectoryTmp, carstate, dwamap, 1);
...
// 计算障碍代价
double DWA::calc_obstacle_cost(const vector<CarState> &trajectory, const CarState &carstate, const AstarPathFinder* tmpmap, int mode)
{
    double distance;
    ...
    //     for(int j = carstate.y - window/2; j <= carstate.y + window/2; j += dwamap->resolution){
