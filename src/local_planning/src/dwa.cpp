...
void DWA::planning()
{
    ...
    if(!first){
        ...
        if(sqrt(pow(currentState.x - myend.x, 2) + pow(currentState.y - myend.y, 2) <= 1.5*tolerance)){
...
double DWA::calc_obstacle_cost(const vector<CarState> &trajectory)
{
    ...
    for (int i = 0; i < mymapmodel.mapinfo.size(); i ++) {
        for (int j = 0; j < trajectory.size(); j ++) {
            ...
            if(distance <= mymapmodel.mapinfo[i].z + radius)
                return 1<<19;
        }
    }
    return 0;
...
int main(int argc, char** argv){
    ...
    n.param<double>("radius", radius, 0.65);
    n.param<double>("tolerance", tolerance, 1.0);
