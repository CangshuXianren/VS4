...
void pathFinding(const Vector2d start_pt, const Vector2d target_pt)
{
    ...
    seg_dis = 2 * overall_length / _astar_path_finder->resolution;
    for(int i = seg_dis; ; i += seg_dis){
        if(i >= grid_path.size()){
            point.pose.position.x = grid_path.back()(0);
            point.pose.position.y = grid_path.back()(1);
            path.poses.push_back(point);
            cout<< "dwa path : " << point.pose.position.x << ", "<<point.pose.position.y<<endl;

            tmp2d<<grid_path.back()(0),grid_path.back()(1);
...
void LocationCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& init_loc_msg){
    ...
    tf::Matrix3x3(quat).getRPY(roll, pitch, yaw);//进行转换
    _yaw = yaw;
    ROS_INFO("[routing] : A* initial : %f, %f, %f", _x, _y, _yaw);
}
